...
sudo apt update
sudo apt install -y build-essential make cmake wl-clipboard git ripgrep python3-pip isort python3-venv libevent-dev ncurses-dev build-essential bison pkg-config xclip jq tidy luarocks bat btop mc mysql-client mysql-server
mkdir -p ~/.local/bin
...
# 3. Neovim specific dependencies
pip install neovim
